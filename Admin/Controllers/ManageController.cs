    ...
    public class ManageController : Controller
    {
        ...
        public ActionResult ForgotPassword(Models.ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Data.User user = manage.GetUserByEmail(model.Email);
                var crypto = new SimpleCrypto.PBKDF2();
                    ...
                    try
                    {
                        if (email.SendEmail(new string[] { ConfigurationManager.AppSettings["EMAIL"] }
                                                    ...
                                                    , "<p>The user with Phone: " + user.Phone + " and email " + user.Email + " forgot his/her password.<p>"
                                                        + "<p>His/her password which was reset is '" + password + "'.</p>"
                                                        + "<p>Please click the <a href='" + Lib.Common.Application.ApplicationPath() + "/account/login/" + user.RegetPasswordCode + "' target='_blank'>link</a> to login the system.</p>"
                                                        + "<br><p><b>DT System</b></p>"
                                                    ))
                        {
                            if (email.SendEmail(new string[] { model.Email }
                                            , "Forget Password"
                                            , "<p>Your password which was reset is '" + password + "'.</p>"
                                              + "<p>Please click the <a href='" + Lib.Common.Application.ApplicationPath() + "/account/login/" + user.RegetPasswordCode + "' target='_blank'>link</a> to login the system.</p>"
                                              + "<br><p><b>DT System</b></p>"
                                            ))
                            {
                                model.Message = "Please check you email to retrieve your password.";
                                return View(model);
                            }
                        }
                    }
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            string result = manage.ValidateAccount(User.Identity.Name, model.OldPassword);
            if (result.Length > 0)
            {
